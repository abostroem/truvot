pro optimal_shift_time,ext,skycounts,backfile,backfileoutput,datafile,x1,y1,x2,y2,override=override,coadd=coadd,ext2=ext2,outputsubtract=outputsubtract,force=force,factor=factor,backscale=backscale
...
;READ IN FITS FILES
if KEYWORD_SET(coadd) then backimage=readfits(backfile,headbackimage,exten_no=ext2) else if KEYWORD_SET(ext2) then backimage=readfits(backfile,headbackimage,exten_no=ext2) else backimage=readfits(backfile,headbackimage)
